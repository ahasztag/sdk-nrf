...
#if defined(CONFIG_BT_HIDS_SCI)
	conn_data->pending_sci_mode = BT_HIDS_SCI_MODE_NONE;
				 ...
				 enum bt_hids_sci_mode_value mode)
{
	...
	if (err) {
		...
		conn_data->pending_sci_mode = mode;
	}
...
static bool sci_conn_update_params_validate_for_mode(
	...
	enum bt_hids_sci_mode_value mode)
{
	...
	if (params->continuation_number != mode_params->continuation_number) {
		LOG_WRN("Continuation number: %u does not match allowed value %u for mode %d",
			params->continuation_number, mode_params->continuation_number, mode);
		return false;
	}

	if (params->supervision_timeout_10ms != mode_params->supervision_timeout_10ms) {
		LOG_WRN("Supervision timeout: %u (x10 ms) does not match allowed value %u (%u ms) "
			"for mode %d",
			params->supervision_timeout_10ms, mode_params->supervision_timeout_10ms,
			(unsigned int)mode_params->supervision_timeout_10ms * 10U, mode);
		return false;
	}
...
{
	int err = 0;
	enum bt_hids_sci_mode_value old_mode = BT_HIDS_SCI_MODE_NONE;

	if (!sci_mode_hids_obj) {
		LOG_WRN("Connection rate change was requested, but no  "
			"HID service found");
		return -ENOENT;
	}
	if (params == NULL || conn == NULL  || new_mode == NULL) {
		return -EINVAL;
	}
	...
	old_mode = conn_data->sci_mode;

	if (status != BT_HCI_ERR_SUCCESS) {
		LOG_ERR("SCI connection rate change failed, status: %d", status);
		conn_data->pending_sci_mode = BT_HIDS_SCI_MODE_NONE;
		*new_mode = conn_data->sci_mode;
		bt_conn_ctx_release(sci_mode_hids_obj->conn_ctx, (void *)conn_data);
		return 0;
	}

	LOG_DBG("SCI connection rate changed, interval: %u us, subrate factor: %d, "
		"peripheral latency: %d, continuation number: %d, supervision timeout: %d ms",
	...
	if (conn_data->pending_sci_mode == BT_HIDS_SCI_MODE_NONE) {
		LOG_INF("No pending SCI mode - verifying if current mode is valid");
		conn_data->pending_sci_mode = conn_data->sci_mode;
	}

	if (sci_conn_update_params_validate_for_mode(params, conn_data->pending_sci_mode)) {
		LOG_INF("New parameters valid for SCI mode %d", conn_data->pending_sci_mode);
		conn_data->sci_mode = conn_data->pending_sci_mode;
	} else {
		LOG_WRN("Defaulting to SCI mode NONE");
		conn_data->sci_mode = BT_HIDS_SCI_MODE_NONE;
		err = -EPIPE;
	}

	if (conn_data->sci_mode != old_mode) {
				     ...
				     uint16_t offset, uint8_t flags)
{
	...
	ssize_t ret = 0;
#if defined(CONFIG_BT_HIDS_SCI_AUTO_CONNECTION_PARAMS_REQUEST)
	int err = 0;
#endif
	...
	case BT_HIDS_CP_EVT_HOST_SCI_FULL_RANGE_REQ:
...
#if defined(CONFIG_BT_HIDS_SCI_AUTO_CONNECTION_PARAMS_REQUEST)
		/* enum bt_hids_sci_mode_value matches corresponding control point event value */
		err = bt_hids_sci_mode_change_request(conn, (enum bt_hids_sci_mode_value) *new_cp);
		...
		case -ENOTSUP:
			...
			break;
		case -EBUSY:
			ret = BT_GATT_ERR(BT_ATT_ERR_PROCEDURE_IN_PROGRESS);
			break;
		case -ENOBUFS:
			ret = BT_GATT_ERR(BT_ATT_ERR_INSUFFICIENT_RESOURCES);
			break;
		default:
			ret = BT_GATT_ERR(BT_ATT_ERR_UNLIKELY);
			break;
		}
		break;
#endif /* CONFIG_BT_HIDS_SCI_AUTO_CONNECTION_PARAMS_REQUEST */
#endif
