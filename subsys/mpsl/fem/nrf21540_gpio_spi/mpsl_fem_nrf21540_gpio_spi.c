...
PINCTRL_DT_DEFINE(MPSL_FEM_SPI_BUS);
#endif
	...
	for (size_t i = 0U; i < ARRAY_SIZE(fem_spi_pin_nums); i++) {
		switch (fem_spi_pin_funcs[i]) {
			case NRF_FUN_SPIM_SCK:
                mpsl_fem_extended_pin_to_mpsl_fem_pin(fem_spi_pin_nums[i], &cfg.spi_config.sck_pin);
				break;
			case NRF_FUN_SPIM_MOSI:
                mpsl_fem_extended_pin_to_mpsl_fem_pin(fem_spi_pin_nums[i], &cfg.spi_config.mosi_pin);
				break;
			case NRF_FUN_SPIM_MISO:
                mpsl_fem_extended_pin_to_mpsl_fem_pin(fem_spi_pin_nums[i], &cfg.spi_config.miso_pin);
				break;
            default:
				break;
		}
