...
* Added:
  ...
  * The :c:struct:`ble_peer_sci_conn_rate_event` event to report connection rate changes or failed connection rate change requests when shorter connection intervals are enabled.
  * The :c:struct:`ble_peer_frame_space_updated_event` event to report frame space changes or failed frame space update requests.
  * The :c:struct:`ble_peer_phy_updated_event` event to report PHY changes.
